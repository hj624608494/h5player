...
Player.prototype = {
  _classes: {
    ...
    play: 'icon-zhutibofang',
    // next: 'fa fa-step-forward',
    pause: 'icon-iconzantingdianji',
  },
  themes: [
    'normal',
    'simple',
  ...
  init: function() {
    ...
    container.innerHTML =
      ...
      '<div data-id="bar" class="uui-audio-progressbar">' +
        '<div class="h5p-bar">' +
          '<div class="h5p-played"></div>' +
        '</div>' +
        '<div class="h5p-cursor"><a></a></div>' +
      '</div>' +
      '<div class="uui-audio-duration clearfix">' +
          '<span class="h5p-curtime"></span>' +
          '<span class="h5p-duration"></span>' +
      '</div>' +
      '<div class="uui-audio-switch">' +
          // '<a class="h5p-button uui-audio-switchbtn">' +
            '<span data-id="play" class="h5p-button iconfont ' + _this.classes.play + '"></span>' +
          // '</a>' +
      '</div>'
    ;
    ...
    _this.prtime = $('.h5p-curtime');
    _this.prduration = $('.h5p-duration');
  ...
  bindEvents: function() {
    ...
    _this.audio.addEventListener('timeupdate', function(e) {
      ...
      _this.prtime.innerHTML = _this.timestr(currentTime);
      _this.prduration.innerHTML = _this.timestr(duration);
  ...
  play: function(i) {
    ...
    $('.J_audio_name').html(song.name);
    $('.J_player_img').css('background', 'url('+song.image.normal+')');
    $('.J_player_img').css('background-size', 'cover');
    $('.J_share_bg').css('background', 'url('+song.image.normal+')');
    $('.J_share_bg').css('background-size', 'cover');
